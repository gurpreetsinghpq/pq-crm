...
function SideSheetDeals({ parentData, permissions }: { parentData: { childData: IChildData, setChildDataHandler: CallableFunction }, permissions: Permission }) {
    ...
    const [isServiceRadioSelected, setIsServiceRadioSelected] = useState<boolean>(data.prospect.lead.service_fee ? true : data.prospect.lead.flat_fee ? false : true)
                ...
                <div className='w-full h-full flex flex-row '>
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem className='w-full'>
                                                            <Select onValueChange={(value) => {
                                                                ...
                                                                <FormControl>
                                                                    <SelectTrigger disabled={rowState?.status==="Lost" || rowState?.status==="Cancelled" } className={`border-gray-300 ${commonClasses}`}>
                    ...
                    <div className='pt-[24px]  2xl:w-3/4 w-8/12 h-full flex-1 flex flex-col'>
                        ...
                        <div className='px-[24px] pb-[24px] flex flex-row bg-gray-50 flex-1 border-t-[1px] border-gray-200 overflow-y-auto overflow-x-hidden '>
                            <SideSheetTabs ids={{accountId:data.prospect.lead.organisation.id}} prospectId={data.prospect.id} title={data.prospect.lead.title} currentParentTab={currentSidesheetTab} contactFromParents={dummyContactData} dealId={data.id} entityId={data.prospect.lead.id} permissions={permissions} disable={{ requirementDeck: false, serviceContract:false }} />
                        </div>
                    </div>
                </div>

...
import { BUDGET_RANGE, COUNTRY_CODE, CURRENCIES, DESIGNATION, DOMAINS, EXCLUSIVITY, INDUSTRY, LAST_FUNDING_AMOUNT, LAST_FUNDING_STAGE, OWNERS, REGION, REGIONS, RETAINER_ADVANCE, ROLETYPE, SEGMENT, SERVICE_FEE_RANGE, SIZE_OF_COMPANY, SOURCES, PROSPECT_STATUSES, TIME_TO_FILL, TYPE, CLOSEDBY, SET_VALUE_CONFIG, SIDE_SHEET_TABS, DUPLICATE_ERROR_MESSAGE_DEFAULT, CONTACT_TYPE } from '@/app/constants/constants'
...
function SideSheetProspects({ parentData, permissions }: { parentData: { childData: IChildData, setChildDataHandler: CallableFunction }, permissions: Permission }) {
    ...
    const form = useForm<z.infer<typeof formSchema>>({
        ...
        defaultValues: {
            ...
            fixedCtcBudgetCurrency: parseCurrencyValue(data.lead.role.fixed_budget || "")?.getCurrencyCode() || getCurrencyAccToRegion(data.lead.role.region),
            fixedBudgetUlCurrency: parseCurrencyValue(data.lead.role.fixed_budget_ul || "")?.getCurrencyCode() || getCurrencyAccToRegion(data.lead.role.region),
            esopRsusUlCurrency: parseCurrencyValue(data.lead.role.esop_rsu || "")?.getCurrencyCode() || getCurrencyAccToRegion(data.lead.role.region),
            flatFeeCurrency: parseCurrencyValue(data.lead.flat_fee || "")?.getCurrencyCode() || getCurrencyAccToRegion(data.lead.role.region),
            equityFeeCurrency: parseCurrencyValue(data.lead.equity_fee || "")?.getCurrencyCode() || getCurrencyAccToRegion(data.lead.role.region),
    ...
    useEffect(() => {
        ...
        form.setValue("billingAddress", formatAddresses(data.lead.organisation).billing)
        form.setValue("shippingAddress", formatAddresses(data.lead.organisation).shipping)
    ...
    useEffect(() => {
        if (isServiceRadioSelected) {
            ...
            form.setValue("flatFeeCurrency", getCurrencyAccToRegion(data.lead.role.region))
                ...
                <div className='w-full h-full flex flex-row '>
                                                            ...
                                                            <Select onValueChange={
                                                                ...
                                                                <FormControl>
                                                                    <SelectTrigger className={`border-none ${commonFontClasses}`}>
                                                                        <SelectValue placeholder={getCurrencyAccToRegion(data.lead.role.region)} />
                                                                    </SelectTrigger>
                                                                </FormControl>
                                                                <SelectContent>
                                                                    {
                    ...
                    <div className='pt-[24px]  2xl:w-3/4 w-8/12 h-full flex-1 flex flex-col'>
                        ...
                        <div className='px-[24px] pb-[24px] flex flex-row bg-gray-50 flex-1 border-t-[1px] border-gray-200 overflow-y-auto overflow-x-hidden '>
                            <SideSheetTabs currentParentTab={currentSidesheetTab} title={data.lead.title} contactFromParents={dummyContactData} prospectId={data.id} entityId={data.lead.id} permissions={permissions} disable={{ requirementDeck: true, serviceContract: true }} />
                        </div>
                    </div>
                </div>

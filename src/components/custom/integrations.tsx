...
function Integrations({ currentOption }: { currentOption: string }) {
    ...
    const [isDocuSignConnected, setDocuSignConnect] = useState<boolean>(false)
    ...
    async function getIntegrationStatus() {
        ...
        try {
            ...
            if (result.data && result.status=="1") {
                ...
                const isDocuSignConnected =data.reverse().find((val)=>val.service_name==="docusign")?.archived === true ? false : true
                console.log(isDocuSignConnected)
                setDocuSignConnect(isDocuSignConnected)
            }else{
                toast({
                    title: `Some error!`,
                    variant: "destructive"
    ...
    return (
        <div className='p-[24px]'>
            {currentOption === CURRENT_OPTION.APPS && <div className='flex flex-row gap-[20px]'>
                <div className='p-[24px] border-[1px] border-gray-300 flex flex-row gap-[10px]'>
                    <div className='rounded-full bg-[#F8F8F8] w-[62px] h-[62px] flex flex-row items-center justify-center p-[5px]'>
                        <img src="./images/docusign-icon.svg" className='object-contain h-full w-full' />
                    </div>
                    <div className='flex flex-col gap-[32px]'>
                        <div className='flex flex-col gap-[12px]'>
                            <div className='flex flex-row items-center justify-between'>
                                ...
                                {isDocuSignConnected && <div className='flex flex-row gap-[5px] bg-success-200 rounded-[38px] text-success-600 p-[8px] text-sm font-semibold'>
                        ...
                        <div>
                            {isDocuSignConnected ? <Button onClick={() => connectDocusign()} >Connect</Button> : <Button className="bg-error-600 hover:bg-error-400">Disconnect</Button>}
                        </div>
                    </div>
                </div>
